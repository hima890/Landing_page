...
import { useTranslation } from 'react-i18next'; // kept for isRTL direction only
...
export default function AdminDashboard({ onBack }: { onBack: () => void }) {
  ...
  const exportPDF = () => {
    ...
    autoTable(doc, {
      startY: 30,
      head: [['Name', 'Business', 'Type', 'WhatsApp', 'Problem', 'Date']],
      body: tableData,
      theme: 'striped',
      ...
      <div className="flex min-h-screen items-center justify-center bg-slate-50 p-4 dark:bg-slate-950">
        ...
        >
          <div className="mb-8 text-center">
            ...
            <h2 className="font-display text-2xl font-bold text-slate-900 dark:text-white">Admin Login</h2>
          ...
          <form onSubmit={handleLogin} className="space-y-4">
            ...
            >
              {isLoggingIn ? <Loader2 className="h-5 w-5 animate-spin" /> : 'Login to Dashboard'}
            </button>
      ...
      <div className="mx-auto max-w-7xl">
        <div className="mb-8 flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
          <div className="flex items-center gap-4">
            ...
            <h1 className="font-display text-2xl sm:text-3xl font-bold text-slate-900 dark:text-white">Admin Dashboard</h1>
          </div>
            ...
            <table className="w-full text-left rtl:text-right">
              <thead>
                <tr className="border-b border-slate-100 bg-slate-50 dark:border-slate-800 dark:bg-slate-900/50">
                  <th className="px-6 py-4 text-sm font-semibold text-slate-900 dark:text-white">Name</th>
                  <th className="hidden px-6 py-4 text-sm font-semibold text-slate-900 dark:text-white md:table-cell">Business</th>
                  <th className="px-6 py-4 text-sm font-semibold text-slate-900 dark:text-white">WhatsApp</th>
                  <th className="hidden px-6 py-4 text-sm font-semibold text-slate-900 dark:text-white lg:table-cell">Problem</th>
